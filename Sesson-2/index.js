...
const filterPostByUid = (id) =>
	console.log(posts.filter((item) => item.userId === id));
...
const findPostById = (id) => {
	...
	// console.log(item ? item : 'Data not found');
};
...
const renderListRandomNumber = () => {
	const items = [];
	Array.from({ length: 1000 }).forEach((_item, index) => {
		index % 2 === 0 && items.push(index);
	});

	console.log(items);
};

// renderListRandomNumber();

const validateProfile = () => {
	/*

  C - tạo mới 
  R - lấy nội dung và hiển thị ra 
  U - cập nhật dữ liệu
  D - xoá
...
const updatePostById = ({ id, title, body }) => {
	...
	if (index !== -1) {
		console.log(`Item befour update: ${posts[index].title}`);

		posts[index] = data;

		console.log(`Item after update: ${posts[index].title}`);
	} else {
		posts.push({
			...data,
			id: 'Mind',
